...
void Merge_Sort_Recursion(int* mas, int r, int t) {
    ...
    int num = i + 1; // количество элементов в 2х сери€х

    int* a = new int[p];//сери€ 1
    int* b = new int[k];//сери€ 2
    ...
    int count = 0;//счетчик
    if (flag == true) { //если есть втора€ сери€
...
int main() {
    ...
    do {
        cout << " аким способом вы хотите создать массив? " << endl;
        cout << "1 - ¬вести ручками" << endl;
        cout << "2 - —оздать случайно" << endl;
        cin >> choice;
    } while (choice < 1 || choice > 2);


    switch (choice) { //выбор способа создани€ массива
    case 1: {
